...
function _findTargetInstagramPage(pages) {
  ...
  if (targetName) {
    ...
    const byName = items.find((item) => {
      const account = item && item.instagram_business_account && typeof item.instagram_business_account === "object"
      ...
      return (
        ...
        name === targetName ||
        (!!targetNorm && (
          norm(username) === targetNorm ||
          norm(name) === targetNorm ||
          norm(username).indexOf(targetNorm) >= 0 ||
          norm(name).indexOf(targetNorm) >= 0
        ))
      );
    });
...
function _normalizeFacebookSelectedPhotos(input) {
...
async function _facebookUploadPhoto(connectionRecord, imageUrl, options) {
...
app.get("/admin/instagram/connect/callback", async (req, res) => {
  ...
  try {
    ...
    const account = page.instagram_business_account && typeof page.instagram_business_account === "object"
